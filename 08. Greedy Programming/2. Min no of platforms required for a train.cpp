class Solution{
    ...
    // TC-O(n log n)+O(n*min_platforms), SC-O(2*n)
    ...
    // 	vector<int> dep_time={a[0].second};
    // 	int i,j;
    // 	for(i=1; i<n; i++){
    // 	    for(j=0; j<dep_time.size(); j++){
    // 	        if(a[i].first>dep_time[j]){
    // 	            dep_time[j] = a[i].second;
    // 	            break;
    // 	        }
    // 	    }
    // 	    if(j>=dep_time.size()){
    // 	        dep_time.push_back(a[i].second);
    // 	    }
    // 	}
